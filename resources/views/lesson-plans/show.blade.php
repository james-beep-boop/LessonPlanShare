<x-layout>
    <x-slot:title>{{ $lessonPlan->class_name }} Lesson {{ $lessonPlan->lesson_day }} — ARES Education</x-slot>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <h1 class="text-2xl font-bold text-gray-900">
                Lesson Plan Details: {{ $lessonPlan->class_name }}: Lesson {{ $lessonPlan->lesson_day }}
            </h1>
        ...
        {{-- Lesson Plan Details box --}}
        <div class="border border-gray-200 rounded-lg p-6">

            {{-- Box header: version + filename (replaces "Lesson Plan Details" h2) --}}
            <div class="mb-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    <span class="text-gray-500">Lesson:</span>
            ...
            @endphp
            <div class="mt-6 pt-4 border-t border-gray-100 space-y-2" x-data="{ confirmOpen: false }">
            ...
            <div class="border border-gray-200 rounded-lg p-6">
                ...
                <div x-data="{
                    ...
                    <div class="flex flex-col sm:flex-row gap-2">
                        <button @click="castVote(1)" type="button" :disabled="loading"
                                :class="userVote === 1
                    ...
                    <div class="flex items-start space-x-3 {{ $version->id === $lessonPlan->id ? 'bg-gray-50 -mx-2 px-2 py-1.5 rounded-md' : '' }}">
                        ...
                        <div class="min-w-0 flex-1">
                            @if ($version->id === $lessonPlan->id)
                                <span class="text-sm font-medium text-gray-900">{{ $version->class_name }} Lesson {{ $version->lesson_day }}</span>
                            ...
                            @else
                                <a href="{{ route('lesson-plans.show', $version) }}"
                                   class="text-sm text-gray-900 hover:text-gray-600 font-medium underline">
                                    {{ $version->class_name }} Lesson {{ $version->lesson_day }}
                                </a>
                            @endif
