<x-layout>
    <x-slot:title>{{ $lessonPlan->name }} - Lesson Plan Exchange</x-slot>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    {{-- Action Buttons --}}
                    <div class="flex items-center space-x-3 mt-6 pt-4 border-t border-gray-100">
                        ...
                        @auth
                            @if ($lessonPlan->author_id === auth()->id())
                                <form method="POST" action="{{ route('lesson-plans.destroy', $lessonPlan) }}"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="px-4 py-2 bg-red-50 text-red-600 text-sm font-medium rounded-md hover:bg-red-100">
                                        Delete
                                    </button>
                                </form>
                            @endif
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    @else
                        <p class="text-sm text-gray-500 border-t border-gray-100 pt-4">
                            <a href="{{ route('login') }}" class="text-indigo-600 hover:text-indigo-800">Log in</a> to vote on this plan.
                        </p>
                    @endauth
                </div>
            </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-3">
                        @foreach ($versions as $version)
                            <div class="flex items-start space-x-3 {{ $version->id === $lessonPlan->id ? 'bg-indigo-50 -mx-2 px-2 py-1.5 rounded-md' : '' }}">
                                ...
                                <div class="min-w-0 flex-1">
                                    @if ($version->id === $lessonPlan->id)
                                        <span class="text-sm font-medium text-indigo-700">{{ $version->class_name }} Day {{ $version->lesson_day }}</span>
                                        <span class="text-xs text-indigo-500 block">Current</span>
                                    @else
