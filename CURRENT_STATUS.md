...
- **[TEMPORARY TESTING]** Show page: collapsible "Alternative Viewers (for testing)" section below the voting card — two buttons open Google Docs Viewer and Microsoft Office Online Viewer in new tabs, for side-by-side comparison. Collapsed by default (dashed border, muted label). Remove once viewer decision is made.
